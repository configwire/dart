...
# NOT idempotent by design: every run uses a FRESH temp dir (/tmp/cn-t17-pbdata,
...
DATA_DIR="/tmp/cn-t17-pbdata"
BIN="/tmp/cn-t17-confignest"
...
"$BIN" superuser upsert "$SU_EMAIL" "$SU_PASS" --dir="$DATA_DIR" >/tmp/cn-t17-upsert.log 2>&1 || {
  fail "superuser upsert ($(cat /tmp/cn-t17-upsert.log | head -c 300))"; exit 1;
}
pass "superuser upsert"
nohup "$BIN" serve --http="127.0.0.1:$PORT" --dir="$DATA_DIR" >/tmp/cn-t17-serve.log 2>&1 &
disown || true
sleep 1
UP=0
...
CODE=$(curl -s -o /tmp/cn-t17-login.json -w "%{http_code}" --max-time 10 -X POST \
...
TOKEN="$(python3 -c "import json; print(json.load(open('/tmp/cn-t17-login.json'))['token'])")"

C="$BASE_URL/api/collections"
[ "$(su_post "$C/projects/records" '{"name":"e2e-proj","owner":"e2e"}' /tmp/cn-t17-proj.json)" = "200" ] \
  && pass "create project" || { fail "create project"; exit 1; }
PROJ="$(pyid /tmp/cn-t17-proj.json id)"
ENV_BODY="{\"project\":\"$PROJ\",\"slug\":\"e2e\"}"
[ "$(su_post "$C/environments/records" "$ENV_BODY" /tmp/cn-t17-env.json)" = "200" ] \
  && pass "create env e2e" || { fail "create env"; exit 1; }
ENVID="$(pyid /tmp/cn-t17-env.json id)"
GROUP_CORE_BODY="{\"name\":\"core\",\"project\":\"$PROJ\"}"
GROUP_GROWTH_BODY="{\"name\":\"growth\",\"project\":\"$PROJ\"}"
[ "$(su_post "$C/groups/records" "$GROUP_CORE_BODY" /tmp/cn-t17-g1.json)" = "200" ] \
  && pass "create group core" || { fail "create group core"; exit 1; }
[ "$(su_post "$C/groups/records" "$GROUP_GROWTH_BODY" /tmp/cn-t17-g2.json)" = "200" ] \
  && pass "create group growth" || { fail "create group growth"; exit 1; }
G1="$(pyid /tmp/cn-t17-g1.json id)"
G2="$(pyid /tmp/cn-t17-g2.json id)"
...
[ "$(mkflag welcome_text string '"hello"' "$G1" /tmp/cn-t17-f1.json)" = "200" ] && pass "flag welcome_text:string" || { fail "flag welcome_text"; exit 1; }
[ "$(mkflag max_items number '10' "$G1" /tmp/cn-t17-f2.json)" = "200" ] && pass "flag max_items:number" || { fail "flag max_items"; exit 1; }
[ "$(mkflag exp_bool bool 'false' "$G2" /tmp/cn-t17-f3.json)" = "200" ] && pass "flag exp_bool:bool" || { fail "flag exp_bool"; exit 1; }
[ "$(mkflag home_config json '{"a":1}' "$G2" /tmp/cn-t17-f4.json)" = "200" ] && pass "flag home_config:json" || { fail "flag home_config"; exit 1; }
F1="$(pyid /tmp/cn-t17-f1.json id)"
F2="$(pyid /tmp/cn-t17-f2.json id)"
F3="$(pyid /tmp/cn-t17-f3.json id)"
F4="$(pyid /tmp/cn-t17-f4.json id)"
...
[ "$(su_post "$C/rules/records" "$RULE1_BODY" /tmp/cn-t17-r1.json)" = "200" ] \
  && pass "rule platform==ios" || { fail "rule platform"; exit 1; }
[ "$(su_post "$C/rules/records" "$RULE2_BODY" /tmp/cn-t17-r2.json)" = "200" ] \
...
[ "$(su_post "$C/experiments/records" "$EXP_BODY" /tmp/cn-t17-exp.json)" = "200" ] \
...
[ "$(su_post "$C/sdk_keys/records" "$SDK_KEY_BODY" /tmp/cn-t17-key.json)" = "200" ] \
  && pass "sdk key (prefix=first8+sha256)" || { fail "sdk key"; exit 1; }

CODE="$(su_post "$BASE_URL/api/v1/admin/env/e2e/publish" '{"note":"t17 v1","baseVersion":0}' /tmp/cn-t17-pub1.json)"
[ "$CODE" = "200" ] && pass "publish v1 200" || { fail "publish v1 (code $CODE)"; exit 1; }
python3 -c "import json,sys; d=json.load(open('/tmp/cn-t17-pub1.json')); sys.exit(0 if d.get('version')==1 else 1)" \
  && pass "publish v1 version==1" || fail "publish v1 version==1"
ETAG_V1="$(python3 -c "import json; print(json.load(open('/tmp/cn-t17-pub1.json'))['etag'])")"
...
import json, sys
d = json.load(open('/tmp/cn-t17-fetch1.json'))
v = d.get('values', {})
...
import json, sys
a = json.load(open('/tmp/cn-t17-fetch1.json'))
b = json.load(open('/tmp/cn-t17-fetch2.json'))
...
sys.exit(0 if ok else 1)
EOF
cp /tmp/cn-t17-fetch1.json /tmp/cn-t17-fetch-stage2.json
...
CODE=$(curl -s -o /tmp/cn-t17-stats.json -w "%{http_code}" --max-time 10 \
...
CODE="$(su_patch "$C/flags/records/$F4" '{"defaultValue":{"a":2}}' /tmp/cn-t17-patch.json)"
[ "$CODE" = "200" ] && pass "flag PATCH default" || { fail "flag PATCH (code $CODE)"; exit 1; }
CODE="$(su_post "$BASE_URL/api/v1/admin/env/e2e/publish" '{"note":"t17 v2 breaking","baseVersion":1}' /tmp/cn-t17-pub2.json)"
...
CODE="$(su_post "$BASE_URL/api/v1/admin/releases/1/rollback" '{"note":"t17 rollback to v1"}' /tmp/cn-t17-rb.json)"
...
import json, sys
d = json.load(open('/tmp/cn-t17-rb.json'))
...
new = json.load(open('/tmp/cn-t17-fetch-v3.json'))
...
if [ "$NEG_EXIT" -ne 0 ]; then
  ...
  if grep -qi "panic\|Traceback" /tmp/cn-t17-neg.json 2>/dev/null; then
...
rm -f /tmp/cn-t17-*.json /tmp/cn-t17-*.log
